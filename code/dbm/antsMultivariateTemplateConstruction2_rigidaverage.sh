...
# This keeps the template shape stable over multiple iterations of template building
    ...
    if [[ $whichtemplate -eq 0 ]];
      then
        ...
        echo "   ${WARP} -d ${dim} -e vector -i ${templatename}0warp.nii.gz -o ${templatename}0warp.nii.gz -t [ ${templatename}0GenericAffine.mat,1 ] -r ${template} --interpolation BSpline[5] --verbose 1"
        echo "--------------------------------------------------------------------------------------"
        ...
        if [[ $NWARPS -ne 0 ]];
          then
            ${WARP} -d ${dim} -e vector -i ${templatename}0warp.nii.gz -o ${templatename}0warp.nii.gz -t [ ${templatename}0GenericAffine.mat,1 ] -r ${template} --interpolation BSpline[5] --interpolation BSpline[5] --verbose 1
    ...
    echo "   ${WARP} -d ${dim} --float $USEFLOAT --interpolation BSpline[5] --verbose 1 -i ${template} -o ${template} -t [ ${templatename}0GenericAffine.mat,1 ] -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz --interpolation BSpline[5] -r ${template}"
    echo "--------------------------------------------------------------------------------------"

    if [[ -f "${templatename}0warp.nii.gz" ]];
      then
        ${WARP} -d ${dim} --float $USEFLOAT --interpolation BSpline[5] --verbose 1 -i ${template} -o ${template} -t [ ${templatename}0GenericAffine.mat,1 ] -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -t ${templatename}0warp.nii.gz -r ${template}
      else
        ${WARP} -d ${dim} --float $USEFLOAT --interpolation BSpline[5] --verbose 1 -i ${template} -o ${template} -t [ ${templatename}0GenericAffine.mat,1 ] -r ${template}
      fi
      ...
      do
        ...
        for (( j = 0; j < $NUMBEROFMODALITIES; j++ ))
          do
            ...
            exe2="$exe2 ${WARP} -d $DIM --float $USEFLOAT --interpolation BSpline[5] --verbose 1 -i ${IMAGESETARRAY[$k]} -o $RIGID -t ${outdir}/rigid${i}_0GenericAffine.mat -r ${TEMPLATES[$j]}\n"
            pexe2="$exe2 ${WARP} -d $DIM --float $USEFLOAT --interpolation BSpline[5] --verbose 1 -i ${IMAGESETARRAY[$k]} -o $RIGID -t ${outdir}/rigid${i}_0GenericAffine.mat -r ${TEMPLATES[$j]} >> ${outdir}/job_${count}_metriclog.txt\n"
          done
          ...
          do
            ...
            if [[ $N4CORRECT -eq 1 ]];
              then
                ...
                warpexe=" $warpexe ${WARP} -d ${DIM} --float $USEFLOAT --interpolation BSpline[5] --verbose 1 -i ${REPAIRED} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS}\n"
                warppexe=" $warppexe ${WARP} -d ${DIM} --float $USEFLOAT --interpolation BSpline[5] --verbose 1 -i ${REPAIRED} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS} >> ${outdir}/job_${count}_metriclog.txt\n"
              else
                ...
                warpexe=" $warpexe ${WARP} -d ${DIM} --float $USEFLOAT --interpolation BSpline[5] --verbose 1 -i ${IMAGESETARRAY[$l]} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS}\n"
                warppexe=" $warppexe ${WARP} -d ${DIM} --float $USEFLOAT --interpolation BSpline[5] --verbose 1 -i ${IMAGESETARRAY[$l]} -o ${DEFORMED} -r ${TEMPLATES[$k]} ${OUTPUTTRANSFORMS} >> ${outdir}/job_${count}_metriclog.txt\n"
              fi
