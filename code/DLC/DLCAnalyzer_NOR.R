...
ReadDLCDataFromCSV_min <- function(file,fps = 1){
  out <- list()
  out$data <- list()
  data.header <- read.table(file, sep =",", header = T, nrows = 1)
  data.header <- data.frame(sapply(data.header, as.character), stringsAsFactors=FALSE)
  raw.data.raw <- read.table(file, sep =",", header = T, skip = 2)
  min <- 110*fps
  raw.data <- raw.data.raw %>% slice(1:min)
  for(i in seq(from = 2, to = nrow(data.header), by = 3)){
    out$data[[paste(data.header[i,])]] <- data.frame(frame = raw.data$coords, x = raw.data[,i], y = raw.data[,(i+1)], likelihood = raw.data[,(i+2)])
  }
...
AddROZones <- function(t, points = c("tl","tr","br","bl"), scale_center = 0.5, scale_corners = 0.4, scale_periphery = 0.8){
  if(!IsTrackingData(t)){
    stop("Object is not of type TrackingData")
  }
  if(length(intersect(points,names(t$data) != 4))){
...
AnalysisRO <- function(t, movement_cutoff,integration_period, points,nosedips = FALSE){
  if(!IsTrackingData(t)){
    stop("Object is not of type TrackingData")
  }
  if(is.null(t$zones)){
    warning("no zones defined for analysis. Returing simple analysis only")
  }
  ...
  #THIS IS A VERA ARBITRARY SECTION FOR THIS TYPE OF ANALYSIS. IT WILL ONLY WORK WITH THE CORRECT POINTS AND ZONES
  if(nosedips){
    if((length(setdiff(c("nose","tailbase","center"),names(t$data))) != 0) | (length(setdiff(c("ob","nob","arena"),names(t$zones))) != 0)){
      warning("Not all points or zones needed for nosedip analysis. Requires points : headcentre,bodycentre,neck and zones closed.top, closed.bottom, arena")
    }else{
      t$labels$automatic.nosedip <- avgbool(!IsInZone(t,"nose","arena") & IsInZone(t,"nose","arena") &!IsInZone(t,"center",c("ob","nob")),integration_period)
      t$Report[["nose.dip"]] <- CalculateTransitions(t$labels$automatic.nosedip,integration_period) / 2
      t$labels$automatic.nosedip <- ifelse(t$labels$automatic.nosedip == 1,"Nosedip","None")
    }
  }
  ...
  for(k in points){
    ...
    t$Report <- append(t$Report, ZoneReport(t,k,"ob", zone.name = paste(k,"ob", sep = ".")))
    t$Report <- append(t$Report, ZoneReport(t,k,"nob", zone.name = paste(k,"nob", sep = ".")))
  }
  return(t)
}
...
PlotDensityPaths <- function(t,points,SDcutoff = 4, Title = "density path"){
  ...
  for(i in points){
    ...
    out[[i]] <- ggplot(data = data_plot, aes(x,y)) + 
      #stat_density_2d_filled(data = data_plot, aes(latbin,longbin, fill=..density..), geom = "raster", contour = FALSE) + 
      #scale_fill_gradient(name = "Time Density", low = "blue", high = "yellow") +
      #scale_fill_distiller(name = "Time Density", palette= "Spectral", direction=-1)+
      #scale_y_continuous(breaks=NULL)+
      geom_path(data = data_plot, aes(x,y, color = SDPlot((speed * t$fps),SDcutoff))) + 
      #ggtitle(paste(i,Title, sep = " ")) 
      scale_color_gradient2(name = paste("speed (",t$distance.units,"/s)",sep = ""), high = "black", low="black", mid = "black")+
      theme_void()+
      #geom_segment(x = 0, y = 0, xend = 40, yend = 0, size = 3) +
      annotate("rect", xmin = izq, xmax = der, ymin = tope, ymax = infe, alpha = 0.3, color = "black", fill = "white", size =.3)+
      theme(legend.position="none")+
      scale_y_reverse()
      #guides(fill = guide_colourbar(fill = guide_colourbar(barwidth = 0.5, barheight = 1)))+
...
PlotDensityPathsRO <- function(t,points,SDcutoff = 4, Title = "density path"){
  if(!IsTrackingData(t)){
    stop("Object is not of type TrackingData")
  }
  out <- NULL
...
AddZonesToPlots <- function(p,z){
  for(i in 1:length(p)){
    for(j in z){
      p[[i]] <- p[[i]] + geom_path(data=j[c(1:nrow(j),1),],aes(x,y), size = 1.51)
    }
  }
  return(p)
}

AddZonesToPlotsRO <- function(p,z){
  for(i in 1:length(p)){
    for(j in z){
      p[[i]] <- p[[i]] + geom_path(data=j,aes(x,y), size = .51)
    }
  }
  return(p)
}
...
PlotPointData <- function(t, points = NULL, from = NULL, to = NULL, unit = "frame", type = NULL){
  ...
  for(i in points){
    p <- p + draw_plot(ggplot(data = t$data[[i]][t$data[[i]]$frame %in% range,], aes(x,y, color = likelihood)) +
                         geom_path() + 
                         ggtitle(i) + 
...
TrackOF <- function(input_folder, OF, OF_Rotataion, name){
  files <- list.files(input_folder)
  TrackingAll <- RunPipeline(files,input_folder,FUN = OF)
  Report <- MultiFileReport(TrackingAll)
  TrackingAll_orientation <- RunPipeline(files,input_folder,FUN = OF_Rotation)
  Report1 <- MultiFileReport(TrackingAll_orientation)
  Reportfinal <- c(Report, Report1[,2:4])
  h <- write.csv(Reportfinal,name)
  
  return(h)
}


TrackingAlls <- function(input_folder, OF, name){
  files <- list.files(input_folder)
  TrackingAll <- RunPipeline(files,input_folder,FUN = OF)
  Report <- MultiFileReport(TrackingAll)
  Reportfinal <- c(Report[,1:8], Report[,15:18], Report[,24:27])
  h <- write.csv(Reportfinal,name)
...
plot_RO <- function(file, diametro, frame, umbra_cutoff) {
  area = diametro/2
  area = sqrt(area^2 + area^2)
  Tracking <- ReadDLCDataFromCSV(file = file, fps = frame)
  names(Tracking$data)
  Tracking <- CleanTrackingData(Tracking, likelihoodcutoff = umbra_cutoff)
  Tracking <- CalibrateTrackingData(Tracking, method = "area",in.metric = area*area, points = c("top","right", "down", "left"))
  PlotPointData(Tracking,points = c("nose"))
  #Tracking$px.to.cm
  centers <- AddROZones(Tracking, scale_center = 0, scale_periphery  = 0 ,scale_corners = 0, points = c("top","right", "down", "left"))
  Tracking <- AddROZones(Tracking, scale_center = 1.5, scale_periphery  = 1.5 ,scale_corners = 0, points = c("oa1","oa2","oa3","oa4"))
  Trackin <- AddROZones(Tracking, scale_center = 1.5, scale_periphery  = 1.5 ,scale_corners = 0, points = c("ob1","ob2","ob3","ob4"))
  #Tracking$zones
  #centers$zones
  #Trackin$zones
  #PlotZones(centers)
  #PlotZones(Tracking)
  #PlotZones(Trackin)
  #PlotZoneVisits(Tracking,point = c("nose"))
  
  Tracking <- CalculateMovement(Tracking, movement_cutoff = 5, integration_period = 5)
  #head(Tracking$data$nose)
  plots <- PlotDensityPaths(Tracking,points = c("nose"))
  plots <- AddZonesToPlots(plots,Tracking$zones)
  plots <- AddZonesToPlots(plots,Trackin$zones)
  p <- plots$nose
  c <- centers$zones$arena      
  
  cx <- (c[4,1]+c[2,1])/2
  cy <- (c[1,2]+c[3,2])/2
  radio = diametro + 1 #radio de setup
  
  r <- p + geom_path(aes(x, y), data = circleFun(c(cx, cy), radio, npoints = 100)) 
...
objet <- function(x, cm){
  ...
  Tracking <- x
  Tracking$zones$ob[[1,1]] <- (Tracking$zones$ob[1, 1]) - cm 
  Tracking$zones$ob[[2,1]] <- (Tracking$zones$ob[2, 1]) - cm
  Tracking$zones$ob[[3,1]] <- (Tracking$zones$ob[3, 1]) + cm
  Tracking$zones$ob[[4,1]] <- (Tracking$zones$ob[4, 1]) + cm
  Tracking$zones$ob[[1,2]] <- (Tracking$zones$ob[1, 2]) + cm
  Tracking$zones$ob[[2,2]] <- (Tracking$zones$ob[2, 2]) - cm
  Tracking$zones$ob[[3,2]] <- (Tracking$zones$ob[3, 2]) - cm
  Tracking$zones$ob[[4,2]] <- (Tracking$zones$ob[4, 2]) + cm
  Tracking$zones$nob[[1,1]] <- (Tracking$zones$nob[1, 1]) - cm
  Tracking$zones$nob[[2,1]] <- (Tracking$zones$nob[2, 1]) - cm
  Tracking$zones$nob[[3,1]] <- (Tracking$zones$nob[3, 1]) + cm
  Tracking$zones$nob[[4,1]] <- (Tracking$zones$nob[4, 1]) + cm
  Tracking$zones$nob[[1,2]] <- (Tracking$zones$nob[1, 2]) + cm
  Tracking$zones$nob[[2,2]] <- (Tracking$zones$nob[2, 2]) - cm
  Tracking$zones$nob[[3,2]] <- (Tracking$zones$nob[3, 2]) - cm
  Tracking$zones$nob[[4,2]] <- (Tracking$zones$nob[4, 2]) + cm
...
ROOverviewPlot <- function(t, point){
  ...
  p1 <- PlotDensityPaths(t,point)
  if(!is.null(t$zones)){
    p1 <- AddZonesToPlots(p1, t$zones[names(t$zones) %in% "arena" == FALSE])
  }
  t1 <- p1[[1]] + scale_y_reverse(limits = c(tope, infe)) + scale_x_continuous(limits=c(izq, der))
  p1 <- t1 + geom_ellipse(aes(x0 = cx, y0 = cy, a = cx2, b = cy2, angle = 0)) 
...
ROOverviewPlotrecorrido <- function(t, point){
  
  #t = TrackingAll$R1RubiADQ2DLC_resnet50_OLMJan22shuffle1_100000.csv
  #point = "nose"
  if(!IsTrackingData(t)){
    stop("Input needs to be a single object of type Tracking")
  }
  ...
  cx2 <- (c[4,1]-c[2,1])/2
  cy2 <- (c[1,2]-c[3,2])/2 
  radio = 50 #radio de setup
  p1 <- PlotDensityPathsRecorrido(t,point)
  if(!is.null(t$zones)){
    #p1 <- AddZonesToPlots(p1, t$zones[names(t$zones) %in% "arena" == TRUE])
    p1 <- AddZonesToPlots(p1, t$zones)
  }
...
PlotDensityPathsRecorrido <- function(t,points,SDcutoff = 4, Title = "density path"){
  if(!IsTrackingData(t)){
    stop("Object is not of type TrackingData")
  }
  out <- NULL
  ...
  for(i in points){
    ...
    out[[i]] <- ggplot(data = data_plot, aes(x,y)) + 
      ...
      #ggtitle(paste(i,Title, sep = " ")) + 
      theme(legend.position="none")+
      scale_color_gradient2(name = paste("speed (",t$distance.units,"/s)",sep = ""), high = "black", mid = "black", low="black")
  }
  return(out)
}